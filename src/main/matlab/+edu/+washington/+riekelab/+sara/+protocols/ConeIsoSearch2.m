classdef ConeIsoSearch2 < edu.washington.riekelab.manookin.protocols.ManookinLabStageProtocol
    ...
    properties
        ...
        spatialFrequency = 4            % Grating spatial frequency
        ...
        numberOfAverages = uint16(117)   % Number of epochs
    end
    
    properties (Hidden)
        ampType
        stimulusClassType = symphonyui.core.PropertyType('char', 'row', {'spot', 'grating'})
    ...
    properties (Hidden) % new 3rd axis params
        blueF1
        blueAxis
        blueMin
        foundBlueMin
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            if ~strcmp(obj.onlineAnalysis, 'none')
                obj.showFigure('edu.washington.riekelab.sara.figures.ConeSpaceFigure', obj.rig.getDevice(obj.amp),...
                    obj.onlineAnalysis, obj.preTime, obj.stimTime, obj.temporalFrequency,... 
                    'theoreticalMins', obj.theoreticalMins, 'stimulusClass', obj.stimulusClass);
        ...
        function MTFanalysis(obj, ~, epoch)
            ...
            for k = 1:numBins
                ...
                index(index > length(responseTrace)) = [];
                ytmp = responseTrace(index);
            ...
            case 'red'
                ...
                obj.redAxis(index) = obj.searchValues(obj.ledIndex);
            ...
            case 'blue'
                ...
                obj.blueAxis(index) = obj.searchValues(obj.ledIndex);
                obj.blueF1(index) = abs(ft(2))/length(ft)*2;
                obj.blueMin = obj.searchValues(find(obj.blueF1==min(obj.blueF1), 3));
            end
            ...
            elseif strcmp(obj.searchAxis, 'blue')
                plot(obj.redAxis, obj.redF1, 'bo-', 'Parent', axesHandle);
            end
            hold(axesHandle, 'off');
        ...
        elseif strcmp(obj.stimulusClass, 'grating')
            grate = stage.builtin.stimuli.Image(uint8(0*rawImage));
            ...
            % make the grating visible only during stimulus time
            grateVisible = stage.builtin.controllers.PropertyController(grate, 'visible',...
                @(state)state.time >= obj.preTime * 1e-3 && state.time < (obj.preTime+obj.state.time)*1e-3);
            p.addController(grateVisible);

            % generate the grating - only drifting for now
        ...
        function prepareEpoch(obj, epoch)
            ...
            elseif (obj.numEpochsCompleted-2*length(obj.searchValues)) == 0
                obj.searchAxis = 'blue';
                if strcmp(obj.onlineAnalysis, 'none')
                    obj.redMin = 0;
                else
            ...
            switch obj.searchAxis
                ...
                case 'green'
                    ...
                    obj.ledIndex = 2;
                case 'blue'
                    index = (obj.numEpochsCompleted+1)-(2*length(obj.searchValues));
                    obj.ledContrasts = [obj.redMin obj.greenMin obj.searchValues(index)];
                    obj.ledIndex = 3;
            end
