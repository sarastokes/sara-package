classdef SaraStageProtocolTest < edu.washington.riekelab.protocols.RiekeLabStageProtocol
% IN DEVELOPMENT - NeuronFigure capabilities
    properties
        interpulseInterval = 0          % Duration between pulses (s)
        useNeuronFigure = false
        neuronFigure = []
    end
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            % Check for a neuronFigure
            if obj.useNeuronFigure
                obj.neuronFigure = obj.getNeuronFigure();
            end
            ...
            try
                obj.showFigure('edu.washington.riekelab.sara.figures.ProgressFigure',...
                    obj.numberOfAverages);
            catch
                % other progress figure
            end
        ...
        function prepareEpoch(obj, epoch)
            ...
            epoch.addParameter('greenLEDName', obj.greenLED);
            if obj.muPerPixel > 0
    ...
    methods % Neuron figure
        function handler = getNeuronFigure(obj)
            % GETNEURONFIGURE  Opens neuron figure
            if ~isempty(obj.neuronFigure)
                handler = obj.neuronFigure();
                obj.neuronFigure.show();
            end

            constructor = str2func('edu.washington.riekelab.sara.figures.NeuronFigure');
            handler = constructor(obj, varargin{:});
            addListener(handler, 'Closed', @(source, event)closedNeuronFigure(obj, src, event));
            obj.neuronFigure = handler;
        end % getNeuronFigure

        function closedNeuronFigure(obj, ~, ~)
            % CLOSEDNEURONFIGURE  Clear neuronFigure property on figure close
            obj.neuronFigure = [];
        end % closedNeuronFigure
    end % neuron figure methods
