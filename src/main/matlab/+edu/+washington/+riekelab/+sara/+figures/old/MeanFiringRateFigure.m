classdef MeanFiringRateFigure < symphonyui.core.FigureHandler
    ...
    methods
        function obj = MeanResponseFigure(device, preTime, varargin)
            co = get(groot, 'defaultAxesColorOrder');
            
            ip = inputParser();
            ip.addParameter('groupBy', [], @(x)iscellstr(x));
            ip.addParamter('sweepColor', co(1,:), @(x)ischar(x) || ismatrix(x));
            ...
            obj.sweepColor = ip.Results.storedSweepColor;
        ...
        function handleEpoch(obj, epoch)
            ...
            if numel(quantities) > 0
                ...
                if strcmp(obj.recordingType, 'extracellular') || strcmp(obj.recordingType, 'spikes_CClamp')
                    y = getInstFt(y, 20);
                    x = linspace(0, length(y), length(y)/sampleRate);
                else
