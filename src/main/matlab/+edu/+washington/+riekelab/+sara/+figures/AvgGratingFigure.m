...
function createUi(obj)
  ...
  setIconImage(clearStoredButton, symphonyui.app.App.getResource('icons/sweep_clear.png')); %NOTE: update;

  seeAchromTraces = uipushtool(...
  	'Parent', toolbar,...
  	'TooltipString', 'See achromatic traces',...
  	'Separator', 'on',...
  	'ClickedCallback', @obj.onSelectedAchromTraces);
...
function onSelectedStoreSweep(obj,~,~)
	if ~isempty(obj.storedData)
		if ishandle(obj.storedData.(obj.chromaticClass))
			% get size
			n = size(obj.storedData.(obj.chromaticClass).F1,1);
			obj.storedData.(obj.chromaticClass).F1(n+1,:) = obj.F1amp;
			obj.storedData.(obj.chromaticClass).P1(n+1,:) = obj.F1phase;
		end
	else
		obj.storedData.(obj.chromaticClass).F1 = obj.F1amp;
		obj.storedData.(obj.chromaticClass).P1 = obj.P1amp;
	end
	% save data by calling storedTraces fcn
	obj.storedTraces(obj.storedData);
end

function onSelectedClearStored(obj,~,~)
	obj.storedTraces('Clear');
end

function onSelectedViewAll(obj,~,~)
	fig.h = figure();
	fig.f1 = axes('parent', fig);
	fig.p1 = axes('parent', fig);

	if ishandle(obj.storedData.achrom)
		fig.af1 = line(obj.xvals, mean(obj.storedData.(obj.chromaticClass).F1), '-ok', 'parent', fig_f1);
		fig.ap1 = line(obj.xvals, mean(obj.storedData.(obj.chromaticClass).P1), '-ok', 'parent', fig_p1);
	end


methods (Static)
function storedData = storedAverages(storedData)
	persistent stored;
	if nargin == 0
		storedData = stored;
	else
		if strcmp(storedData, 'Clear')
			stored = [];
		else
			stored = storedData;
			storedData = stored;
